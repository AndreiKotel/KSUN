...
function TFormSearchOP.Seach():integer;
begin
  s:=0;
  wwQuery1.Open;
  Edit2.Text:='';
...
procedure TFormSearchOP.Edit1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
...
procedure TFormSearchOP.wwDBGrid1KeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key=chr(13) then begin s:=wwQuery1Code.Value; Close; end;
end;

procedure TFormSearchOP.FormActivate(Sender: TObject);
begin
  Edit2.SetFocus;
end;

procedure TFormSearchOP.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key=chr(27) then Close;
end;

procedure TFormSearchOP.wwDBGrid1DblClick(Sender: TObject);
begin
